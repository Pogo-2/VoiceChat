...
impl JoinRoomUseCase {
    ...
    ///
    /// Returns the **server-assigned PeerId** for this client.
    pub async fn execute(
        ...
        // --- 3. Wait for PeerList ---
        let our_peer_id = loop {
            let msg = self.transport.recv_control().await?;
            match msg {
                ControlMsg::PeerList { your_peer_id, peers } => {
                    info!(?your_peer_id, count = peers.len(), "Received PeerList");
                    for peer in peers {
